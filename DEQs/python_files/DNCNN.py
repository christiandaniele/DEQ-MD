...
class DnCNN(Denoiser):
    ...
    def __init__(
        ...
        self.nl_list = nn.ModuleList([nn.Softplus(beta=100) for _ in range(self.depth - 1)])
    ...
    def forward(self, x, sigma=None):
        ...
        # y=F.softplus(self.out_conv(x1) + x)*torch.sqrt(torch.sum(self.tv.nabla(x) ** 2 , dim=-1)+1e-6)
